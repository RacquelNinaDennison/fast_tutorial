...
const withRefResolver = options => ({ ...options,
...
const register = async (f, {
  ...
  if (swaggerOptions) {
    ...
    await f.register(_swagger.default, withRefResolver(baseSwaggerOptions));

    if (transformSpec) {
      const originalRoutePrefix = baseSwaggerOptions.routePrefix ?? `/documentation`;
  ...
  const addRoute = ({
    ...
    f[method](url, {
      schema: {
        ...
        querystring: querystring ? $ref(querystring) : undefined,
        response: reply ? {
          200: $ref(reply)
        } : undefined,
        ...fastifySchema
